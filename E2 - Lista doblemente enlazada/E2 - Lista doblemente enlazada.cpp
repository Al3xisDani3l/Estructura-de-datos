// E2 - Lista doblemente enlazada.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
...
#pragma region Definicion del nodo doble
...
public:
    ...
    T &operator[](int index) {
        ...
        if (!isEmptyList()) {
            if (!isArrayOutOfBounds(index)) {
                int actualIndex = 0;
                for (Nodo<T>* node = _cabeza; node != NULL; node = node->Siguiente) {
    ...
    void Add(int index, T value) {
        if (index == Count()) {
            ...
            if (!isEmptyList()) {
                if (!isArrayOutOfBounds(index)) {
                    int actualIndex = 0;
                    for (Nodo<T>* actualNode = _cabeza; actualNode != NULL; actualNode = actualNode->Siguiente) {
    ...
    int IndexOf(T value) {
        if (!isEmptyList()) {
            int index = 0;
            for (Nodo<T>* node = _cabeza; node != NULL &&  index < _count; node = node->Siguiente) {
                if (node->Dato == value) {
                    return index;
                }
                index++;
    ...
    int Count() {
        if (IsEmpty()) {
            ...
            int size = 1;
            for (Nodo<T>* node = _cabeza; node != _final; node = node->Siguiente) {
                size++;
            }
            if (size != _count){
...
int main()
{
    ...
    do {
        ...
        cout << endl << endl << "Menu Lista doblemente enlazada" << endl <<
            ...
            "13 Borrar un nodo segun su posicion" << endl <<
            "14 Borrar un nodo segun su numero" << endl <<
            "15 Borrar el primer nodo de la lista" << endl <<
            "16 Borrar el ultimo nodo de la lista" << endl <<
            "0 Salir" << endl << endl;
        cin >> opc;
        switch (opc)
        {
        ...
        case 13:
            cout << endl << "Introdusca la posicion del nodo a eliminar: ";
            cin >> pos;
        ...
        case 14:
            ...
            cin >> dato;
            cout << endl << "Se eliminara el nodo en la posicion " << lista->IndexOf(dato) << " !" << endl;
